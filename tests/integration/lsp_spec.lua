...
-- default `make test` run stays JDK-free. CI runs these on tagged pushes.
...
describe("souther-lsp integration", function()
  ...
  it("attaches with the pom.xml directory as root", function()
    assert.truthy(wait_for(function()
      return #vim.lsp.get_clients({ bufnr = buf, name = "souther" }) > 0
    end))
    local client = vim.lsp.get_clients({ bufnr = buf, name = "souther" })[1]
